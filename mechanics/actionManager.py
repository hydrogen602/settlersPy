...
#     @abstractmethod
#     def doAction(self, messageObject: Message) -> None:
#         '''
#         Called by `ActionManager` to run an action.
#         The method needs to verify wether or not this is legal according to the game rules.
#         The method should directly send error messages to the player.
...
#         def __init__(self, doAction: Callable[[Message], None], group: str, name: str):
#             self.__doAction: Callable[[Player], None] = doAction
#             super().__init__(group, name)
        
#         def doAction(self, messageObject: Message) -> None:
#             '''
#             Called by `ActionManager` to verify and run an action.
#             This is just a wrapper over the passed
#             in `doAction` function that gives annotations.
#             '''
#             return self.__doAction(messageObject)
...
#     @classmethod
#     def call(cls, group: str, name: str, messageObject: Message) -> None:
#         '''
...
#         ac.doAction(messageObject=messageObject)
